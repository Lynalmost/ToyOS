...
; 虽然每个中断的处理程序可能各不相同,但是有些操作是一样的,所以为了避免代码的重复以及保持简洁
; 我们将压入错误号和中断号的共同部分抽取出来,分成一部分，将之后的保护信息的操作分成一部分
; 然后将处理程序分成一部分,最后将恢复信息分成一部分。
; 一共四个部分,其中除了处理程序不同,其余的部分基本相同(由于有些中断没有错误号,所以需要特殊处理)
